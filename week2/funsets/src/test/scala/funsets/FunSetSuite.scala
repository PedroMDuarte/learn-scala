...
class FunSetSuite extends FunSuite {
  ...
  test("intersection contains all elements of each set") {
    new TestSets {
      ...
      assert(!contains(s, 3), "Intersection 1")

      val t = intersect(union(union(s1, s2), s3), union(s1, s3))
      assert(contains(t, 1), "Intersection 1")
      assert(!contains(t, 2), "Intersection 2")
      assert(contains(t, 3), "Intersection 2")
